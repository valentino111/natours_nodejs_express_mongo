...
exports.restrictTo =
  (...roles) =>
  (req, res, next) => {
    // roles [''adim, 'lead-guuide']. role='user'
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) If token has not expired, and there is a user, set theh new password
  if (!user) {
    return next(new AppError('Token is invalid or hes expired', 400));
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new AppError(
        'Please provide email, password, newPassword and newPasswordConfirm!',
        400
      )
    );
  }
